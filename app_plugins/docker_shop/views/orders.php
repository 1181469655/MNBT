...
$page_title = $page_title ?? 'Docker 订单';
$orders = $orders ?? ['list' => [], 'total' => 0, 'page' => 1, 'per_page' => 15];
$status_labels = ['pending'=>'待支付','paid'=>'已支付','opened'=>'已开通','failed'=>'失败','cancelled'=>'已取消'];
ob_start();
?>
<div class="hs-section"><h1>Docker 订单</h1><p>Docker 购买订单记录</p></div>

<div class="layui-card">
  <div class="layui-card-body" style="padding:0;">
    <?php if (empty($orders['list'])): ?>
      <p style="text-align:center;padding:40px;color:#999;">暂无订单，<a href="<?= htmlspecialchars(docker_shop_url('docker-shop'), ENT_QUOTES) ?>" style="color:#1e9fff;">去购买 Docker</a></p>
    <?php else: ?>
        ...
        <tbody>
          <?php foreach ($orders['list'] as $o): ?>
            <tr>
              ...
              <td><?= htmlspecialchars(docker_shop_periods()[$o['period']]['label'] ?? $o['period']) ?></td>
              <td>¥<?= docker_shop_format_cents($o['amount_cents']) ?></td>
      ...
      if($tp>1): ?>
        <div class="ly-pager">
          <?php if($cp>1):?><a href="<?= htmlspecialchars(docker_shop_url('docker-shop/orders?page='.($cp-1)), ENT_QUOTES) ?>">上一页</a><?php endif;?>
          <span class="ly-pager-info">第<?= $cp ?>/<?= $tp ?>页</span>
          <?php if($cp<$tp):?><a href="<?= htmlspecialchars(docker_shop_url('docker-shop/orders?page='.($cp+1)), ENT_QUOTES) ?>">下一页</a><?php endif;?>
        </div>
      <?php endif;?>
    <?php endif; ?>
  </div>
