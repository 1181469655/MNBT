...
$orders = docker_shop_order_list_all($page, 30, $filters);
...
<div class="container-fluid p-t-15">
	<div class="card">
		<div class="card-header">
			<h4 style="display:inline-block">Docker 订单管理</h4>
		</div>
		<div class="card-body">
			<form method="get" class="form-inline mb-3">
				<input type="hidden" name="p" value="docker_shop">
					...
					<tbody>
						...
						<?php else: ?>
							<?php foreach ($orders['list'] as $o):
								$docker_user = $o['docker_user_id'] > 0 ? $DB->get_row_prepare("SELECT username FROM MN_docker_user WHERE id=? LIMIT 1", [(int)$o['docker_user_id']]) : null;
							?>
							<tr>
								<td><?= (int)$o['id'] ?></td>
								...
								<td><?= htmlspecialchars((docker_shop_periods()[$o['period']]['label'] ?? $o['period']), ENT_QUOTES) ?></td>
								<td>¥<?= htmlspecialchars(docker_shop_format_cents($o['amount_cents'])) ?></td>
								<td><?= htmlspecialchars($nodeMap[(int)$o['node']] ?? (int)$o['node'], ENT_QUOTES) ?></td>
								<td><?= $docker_user ? htmlspecialchars($docker_user['username'], ENT_QUOTES) : '-' ?></td>
								<td>
			...
			if ($total_pages > 1):
				$qs = http_build_query(['p' => 'docker_shop', 'page' => 'orders', 'status' => $filters['status'], 'order_no' => $filters['order_no'], 'user_id' => $filters['user_id']]);
			?>
				<nav>
