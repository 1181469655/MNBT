...
> 状态：评审中
> 分支：dev/v1.84
> 关联文档：[THEME_DEV.md](../templates/THEME_DEV.md)、[API.md](../API.md)、`app_plugins/shop_frontend/README.md`（现有主页实现）
...
- 访问 `/` → [index.php](../index.php) 依次执行：插件通用路由 → 插件首页接管（[`mnbt_plugin_dispatch_home()`](../MPHX/plugin.php#L1209-L1251)）→ 默认 `header("Location:user")`
