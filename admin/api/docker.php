...
if ($egn === 'docker_plan_add') {
	...
	$proxy_max = daddslashes($_POST['proxy_max'] ?? '0');
	$jg = daddslashes($_POST['jg'] ?? '0');
	$qk = daddslashes($_POST['qk'] ?? 'true');
	if ($name === '') json_exit('套餐名不能为空');
	if ($DB->query_prepare("INSERT INTO MN_docker_plan (name,jc,cpu_max,mem_max,disk_max,proxy_max,jg,qk,date) VALUES (?,?,?,?,?,?,?,?,?)", [$name, $jc, $cpu_max, $mem_max, $disk_max, $proxy_max, $jg, $qk, $date])) {
...
if ($egn === 'docker_plan_edit') {
	...
	$qk = daddslashes($_POST['qk'] ?? 'true');
	if ($id <= 0) json_exit('参数错误');
	if ($DB->query_prepare("UPDATE MN_docker_plan SET name=?,jc=?,cpu_max=?,mem_max=?,disk_max=?,proxy_max=?,jg=?,qk=? WHERE id=?", [$name, $jc, $cpu_max, $mem_max, $disk_max, $proxy_max, $jg, $qk, $id])) {
...
if ($egn === 'docker_options') {
	...
	$plans = $DB->get_all_prepare("SELECT id,name,cpu_max,mem_max,disk_max,proxy_max,jg,qk FROM MN_docker_plan WHERE qk='true' ORDER BY id") ?: [];
