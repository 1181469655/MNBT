<template>
  <div class="td-page">
    <div class="td-page-head">
      <div>
        <h3 class="td-page-title"><i class="mdi mdi-docker"></i> Docker 商城</h3>
        <p class="td-page-subtitle">选择适合你的 Docker 容器套餐，支付后自动开通</p>
      </div>
      <t-button variant="outline" @click="router.push('/docker-assets')">
        <i class="mdi mdi-docker"></i> 我的 Docker
      </t-button>
    </div>
    ...
    <div v-if="plans.length" class="plan-grid">
      <div class="plan-card td-card" v-for="plan in plans" :key="plan.id">
        ...
        <ul class="plan-specs">
          <li><i class="mdi mdi-chip"></i><span>CPU 核心</span><b>{{ cpuText(plan) }}</b></li>
          <li><i class="mdi mdi-memory"></i><span>内存</span><b>{{ memText(plan) }}</b></li>
          <li><i class="mdi mdi-harddisk"></i><span>磁盘</span><b>{{ diskText(plan) }}</b></li>
          <li><i class="mdi mdi-shield-lock-outline"></i><span>代理</span><b>{{ proxyText(plan) }}</b></li>
          <li><i class="mdi mdi-server-network"></i><span>开通节点</span><b>{{ plan.node?.name || '—' }}</b></li>
        </ul>
    ...
    <t-empty v-else description="暂无可购买的 Docker 套餐" style="padding: 60px 0" />
...
import { getDockerShopPlans, getDockerShopMethods, createDockerShopOrder, centsToYuan, goPay } from '@/account/api/plugins'
...
function cpuText(plan) {
  const v = Number(plan.base_plan?.cpu_max)
  return v > 0 ? v + ' 核' : '—'
}

function memText(plan) {
  const mb = Number(plan.base_plan?.mem_max)
  if (!mb) return '—'
  return mb >= 1024 ? (mb / 1024) + ' GB' : mb + ' MB'
}

function diskText(plan) {
  const mb = Number(plan.base_plan?.disk_max)
  if (!mb) return '—'
  return mb >= 1024 ? (mb / 1024) + ' GB' : mb + ' MB'
}

function proxyText(plan) {
  const v = Number(plan.base_plan?.proxy_max)
  return v > 0 ? v + ' 个' : '不限'
}
...
async function onConfirmOrder() {
  ...
  const res = await createDockerShopOrder(currentPlan.value.id, period.value, payType.value)
...
async function load() {
  const [plansRes, methodsRes] = await Promise.all([getDockerShopPlans(), getDockerShopMethods()])
