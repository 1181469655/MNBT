...
/**
 * MNBT 主页/售卖 SPA 路由 API 封装
 *
 * 插件（user_info / balance / hosting_shop）通过 P2 通用路由暴露 JSON API：
...
export async function routeRequest(method, path, data = {}, { silent = false } = {}) {
  try {
    ...
    console.error(`[homeApi] ${method} ${path} 异常:`, e, body)
...
/** 未登录统一处理：返回 true 表示已拦截跳转 */
export function handleNotLogin(result) {
